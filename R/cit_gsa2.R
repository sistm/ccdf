cit_gsa2 <- function(M,
  ...
  stopifnot(test=="asymptotic") # enlever quand aura fait pour permutations
  ...
  if (test=="permutation"){
    ...
    if(is.vector(genesets)){
      ...
      for (i in 1:length(genesets)){
        ...
        decomp[[i]] <- eigen(Sigma[[i]], symmetric=TRUE, only.values=TRUE)
      ...
      pval <- survey::pchisqsum(sum(test_stat_gs), lower.tail = FALSE, df = rep(1,length(decomp_vec)), 
                                a = decomp_vec, method = "saddlepoint")
