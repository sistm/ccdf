    ...
    if(is.vector(genesets)){
      ...
      for (i in 1:nrow(Sigma2)) {
        for (j in 1:ncol(Sigma2)) {
          ...
          prop_i <- ifelse(i <= length(prop_gs_vec), prop[i], prop[i - length(prop_gs_vec)])
          prop_j <- ifelse(j <= length(prop_gs_vec), prop[j], prop[j - length(prop_gs_vec)])
