...
cit_asymp_for_sigma <- function(Y, X, Z = NULL, space_y = FALSE, number_y = length(unique(Y))){
  ...
  Sigma2 <- matrix(NA,length(prop),length(prop)) 
  colnames(Sigma2) <- 1:length(prop)
  # 1ère ligne fait toutes les colonnes, puis 2ème ligne ...
  for (i in 1:nrow(Sigma2)){ 
    for (j in 1:ncol(Sigma2)){ 
      if (i<=j){ Sigma2[i,j] <- 1/n_Y_all* tcrossprod(H)  * (prop[i] - prop[i]*prop[j])
      } else { Sigma2[i,j] <- 1/n_Y_all* tcrossprod(H) * (prop[j] - prop[i]*prop[j])}
    }
  } # => avec ma boucle obtient Sigma pas Sigma 2
  decomp <- eigen(Sigma2, symmetric=TRUE, only.values=TRUE)
  pval <- survey::pchisqsum(test_stat, lower.tail = FALSE, df = rep(1, ncol(Sigma2)), 
