...
cit_gsa_h_pi <- function(M,
  ...
  if (test=="permutation"){
    ...
    if(is.vector(genesets)){
      ...
      prop_gs <- list()
      indi_pi_gs <- matrix(NA, n*length(genesets), (p-1)) 
      ...
      cross_H <- tcrossprod(H)
      
      if (nrow(H)>1){ # 2 conditions
        for (i in 1:nrow(Sigma2)) {
          for (j in 1:ncol(Sigma2)) {
            if (i <= length(prop_gs_vec) && j <= length(prop_gs_vec)) {
              cross_val <- cross_H[1, 1]
            } else if (i > length(prop_gs_vec) && j <= length(prop_gs_vec)) {
              cross_val <- cross_H[2, 1]
            } else if (i <= length(prop_gs_vec) && j > length(prop_gs_vec)) {
              cross_val <- cross_H[1, 2]
            } else {
              cross_val <- cross_H[2, 2]
            }
            
            prop_i <- ifelse(i <= length(prop_gs_vec), prop_gs_vec[i], prop_gs_vec[i - length(prop_gs_vec)])
            prop_j <- ifelse(j <= length(prop_gs_vec), prop_gs_vec[j], prop_gs_vec[j - length(prop_gs_vec)])
            
            if (i <= j) {
              Sigma2[i, j] <- 1/n_Y_all * cross_val * (prop_i - prop_i * prop_j)
            } else {
              Sigma2[i, j] <- 1/n_Y_all * cross_val * (prop_j - prop_i * prop_j)
            }
          }
        }
        
      } else { # 1 seule condition
        for (i in 1:nrow(Sigma2)) { 
          for (j in 1:ncol(Sigma2)) {
            if (i <= j) {
              Sigma2[i, j] <- 1/n_Y_all * cross_H * (prop_gs_vec[i] - prop_gs_vec[i] * prop_gs_vec[j])
            } else {
              Sigma2[i, j] <- 1/n_Y_all * cross_H * (prop_gs_vec[j] - prop_gs_vec[i] * prop_gs_vec[j])
            }
          }
        }
      }
      ...
      pval <- survey::pchisqsum(sum(test_stat_gs), lower.tail = FALSE, df = rep(1, ncol(Sigma)),
                                a = decomp$values, method = "saddlepoint") # decomp$d
